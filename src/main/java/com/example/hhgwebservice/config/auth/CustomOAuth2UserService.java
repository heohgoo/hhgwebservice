...
import com.example.hhgwebservice.config.auth.dto.OAuthAttributes;
import com.example.hhgwebservice.config.auth.dto.SessionUser;
import com.example.hhgwebservice.domain.user.User;
import com.example.hhgwebservice.domain.user.UserRepository;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthorizationException {
        ...
        String registrationId = userRequest.getClientRegistration()
                .getRegistrationId();
        //로그인 진행 중인 서비스 구분하는 코드(구글, 네이버...)

        String userNameAttributeName = userRequest.getClientRegistration()
                .getProviderDetails().getUserInfoEndpoint().getUserNameAttributeName();
        //OAuth2 로그인 시 키가 되는 필드값(like PK), 구글은 코드 지원하지만 네이버, 카카오는 x
        ...
        private User saveOrUpdate(OAuthAttributes attributes) {
            ...
            //update 기능 구현, 사용자의 이름이나 사진이 변경되면 User엔티티에도 반영
